...
On Error Resume Next
    ...
    Set XMLpart = wbTarget.CustomXMLParts("CRACustomXMLPart")
    
    If Len(XMLpart.XML) = 0 Then 'default value, model has not been injected into another workbook
        Injected = 0
    ElseIf XMLpart.SelectSingleNode("//ns0:CRACustomElement").Text = "InjectedModel" Then 'model injected
        Injected = 1
    End If
...
On Error GoTo ErrorWorkbook:
    ...
    If (Application.Version() = 14) And (Version = 2013) Then
        If MsgBox("Excel 2010 cannot work with PowerPivot 2013 models directly." & Chr(10) & Chr(10) & _
            ...
            wbTarget.CustomXMLParts.Add ("<CRACustomElement xmlns=""CRACustomXMLPart""><![CDATA[InjectedModel]]></CRACustomElement>")
            wbTarget.Save
